/* References
Checkerboard coefficient paper:
Hopman, J. A., Santos, D., Alsalti-Baldellou, À., Rigola, J., Trias, F. X.
(2025). Quantifying the checkerboard problem to reduce numerical dissipation.
Journal of Computational Physics, 521, 113537.
...
#include "faceToCellInterpolate.H"
#include "quantifyCheckerboard.H"
...
int main(int argc, char *argv[])
{
    ...
    const surfaceScalarField OmegaS = fvc::createOmegaS(mesh);
    double pcb    = 0;
    double PotEcb = 0;
    ...
    while (runTime.run())
    {
        ...
        // Apply dynamic predictor values using the checkerboard coefficient
        if (dynamicpnPredCoef)
        {
            pcb = fvc::quantifyCheckerboard(p, OmegaS);
            pnPredCoefOld = pnPredCoef;
            pnPredCoef = (1.0 - pcb + pnPredCoefOld)/2.0;
        }

        if (dynamicPotEnPredCoef)
        {
            PotEcb = fvc::quantifyCheckerboard(PotE, OmegaS);
            PotEnPredCoefOld = PotEnPredCoef;
            PotEnPredCoef = (1.0 - PotEcb + PotEnPredCoefOld)/2.0;
        }
        ...
        // Quantify the checkerboard coefficient for pressure and electric potential
        pcb    = fvc::quantifyCheckerboard(p,    OmegaS);
        PotEcb = fvc::quantifyCheckerboard(PotE, OmegaS);
